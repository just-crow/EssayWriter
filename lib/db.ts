...
/**
 * Chainable offline stub: every method call returns another stub, awaits
 * resolve to it, and destructured results come back undefined — so every
 * caller drops into its existing memory-db fallback without a single
 * type change. Used when no Supabase key is configured (local work stays
 * fully functional offline).
 */
function offlineStub(): SupabaseClient {
...
export const supabase: SupabaseClient = supabaseKey
